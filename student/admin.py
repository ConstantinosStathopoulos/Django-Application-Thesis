...
#Action Description
set_status_accepted.short_description = "Accept Selected Payments"

#deny payment
def set_status_denied(modeladmin, request,queryset):
    queryset.update(status = 'Denied')

set_status_denied.short_description = "Deny Selected Payments"

class PaymentA(admin.ModelAdmin):
    list_display = ('user','date','document' , 'status')
